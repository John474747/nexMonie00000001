...
import { UtilityLauncher } from '@/components/utilities/UtilityLauncher'
import { UTILITIES_CONFIG } from '@/config/utilities-config'
...
export default function UtilitiesHub() {
  ...
  const { launchUtility: launchExternal } = useUtilityLauncher()

  const launchUtility = (serviceId: string, icon: React.ReactNode) => {
    const service = UTILITIES_CONFIG.find(s => s.id === serviceId);
    if (!service || !service.enabled) return;

    if (service.requiresExternalBrowser) {
        launchExternal(serviceId)
    } else {
        setActiveApp({ name: service.name, icon, url: service.url });
    }
  }
        ...
        <Card className="p-4 border-none shadow-soft rounded-[16px] bg-white">
          ...
          <div className="flex items-center justify-between">
            <div className="flex gap-4">
              ...
              <BrandItem 
                label="Apple Music"
                onClick={() => launchUtility("apple-music", (
              ...
              <BrandItem 
                label="Audiomack"
                onClick={() => launchUtility("audiomack", (
              ...
              <BrandItem 
                label="Boomplay"
                onClick={() => launchUtility("boomplay", (
        ...
        <Card className="p-4 border-none shadow-soft rounded-[16px] bg-white">
          ...
          <div className="flex items-center justify-between">
            <div className="flex gap-4">
              ...
              <BrandItem 
                label="Telegram"
                onClick={() => launchUtility("telegram", (
      ...
      <div className="px-4 grid grid-cols-2 gap-3 mb-4">
        <Card className="p-4 border-none shadow-soft rounded-[16px] bg-[#FFF4EE]">
          ...
          <div className="grid grid-cols-2 gap-y-3 mt-4">
            <SmallBrand 
              onClick={() => launchUtility("chowdeck", <div className="w-9 h-9 rounded-full bg-[#FFD700] flex items-center justify-center text-[#4A2F1D] font-black text-lg">C</div>)}
              logo={<div className="w-9 h-9 rounded-full bg-[#FFD700] flex items-center justify-center text-[#4A2F1D] font-black text-lg">C</div>} 
              label="Chowdeck" 
            />
            <SmallBrand 
              onClick={() => launchUtility("bolt", <div className="w-9 h-9 rounded-md bg-white flex items-center justify-center text-[#34D186] font-bold text-[9px] tracking-tighter">Bolt</div>)}
            ...
            />
            <SmallBrand 
              onClick={() => launchUtility("indrive", <div className="w-9 h-9 rounded-full bg-white flex items-center justify-center text-[#00C853] font-black text-[12px] border border-gray-100 shadow-sm">iD</div>)}
            ...
            />
            <SmallBrand 
              onClick={() => launchUtility("rida", <div className="w-9 h-9 rounded-md bg-[#6C3CE1] flex items-center justify-center text-white font-bold text-lg">R</div>)}
        ...
        <Card className="p-4 border-none shadow-soft rounded-[16px] bg-[#FFF0F0]">
          ...
          <div className="grid grid-cols-3 gap-y-3 mt-4">
             <div onClick={() => launchUtility("aliexpress", <span>AE</span>)} className="w-8 h-8 rounded-md bg-[#FF4747] flex items-center justify-center text-white text-[9px] font-black cursor-pointer transition-transform active:scale-90">AE</div>
             <div onClick={() => launchUtility("temu", <span>T</span>)} className="w-8 h-8 rounded-md bg-[#FF4F00] flex items-center justify-center text-white text-[10px] font-bold cursor-pointer transition-transform active:scale-90">T</div>
             <div onClick={() => launchUtility("amazon", <span>a</span>)} className="w-8 h-8 rounded-md bg-white flex flex-col items-center justify-center shadow-sm cursor-pointer transition-transform active:scale-90">
                <span className="text-black font-bold text-[10px] leading-none">a</span>
                <div className="w-5 h-[2px] bg-[#FF9900] rounded-full mt-[1px]" />
             </div>
             <div onClick={() => launchUtility("uber", <span>Uber</span>)} className="w-8 h-8 rounded-md bg-black flex items-center justify-center text-white text-[8px] font-bold tracking-tight cursor-pointer transition-transform active:scale-90">Uber</div>
          </div>
        </Card>
      </div>
      ...
      <div className="px-4 grid grid-cols-2 gap-3 mb-6">
        <Card className="p-4 border-none shadow-soft rounded-[16px] bg-[#E8F5F3]">
          ...
          <div className="flex flex-wrap gap-2 mt-4">
            <div onClick={() => launchUtility("booking", <span>B</span>)} className="w-8 h-8 rounded-md bg-[#003580] flex items-center justify-center text-white font-black text-xs cursor-pointer active:scale-90">B</div>
            <div onClick={() => launchUtility("canva", <span>C</span>)} className="w-8 h-8 rounded-full bg-[#00C4CC] flex items-center justify-center text-white font-black text-xs cursor-pointer active:scale-90">C</div>
            <div onClick={() => launchUtility("zoom", <svg width="14" height="14" viewBox="0 0 24 24" fill="white"><path d="M17 10.5V7c0-.55-.45-1-1-1H4c-.55 0-1 .45-1 1v10c0 .55.45 1 1 1h12c.55 0 1-.45 1-1v-3.5l4 4v-11l-4 4z"/></svg>)} className="w-8 h-8 rounded-md bg-[#2D8CFF] flex items-center justify-center text-white cursor-pointer active:scale-90">
            ...
            <div onClick={() => launchUtility("gmail", <span>M</span>)} className="w-8 h-8 rounded-md bg-white shadow-sm flex items-center justify-center cursor-pointer active:scale-90">
            ...
            <div onClick={() => launchUtility("m365", <span>MS</span>)} className="w-8 h-8 rounded-md bg-white shadow-sm flex items-center justify-center p-1 cursor-pointer active:scale-90">
      ...
      <div className="px-4 mb-8">
        <Card className="p-5 border-none bg-[#005F56] rounded-[24px] flex items-center justify-between">
          ...
          <button 
            ...
            className="px-4 py-2 border-[1.5px] border-white rounded-full text-[13px] font-bold text-white bg-transparent active:bg-white/10 transition-colors cursor-pointer"
          >
            Add Money →
          </button>
        </Card>
